...
def processData(dataset, singular_matrix, Mu, BigSigma):
...
def generate_mu_var(dataset, singular_matrix):
    M = 10
    data = pd.read_csv(dataset)
    data = data.drop(data.columns[[0, 1, -1]], axis=1)
    data = data.drop(data.columns[singular_matrix], axis=1)
...
def GenerateBigSigma(Data):
    ...
    for i in range(0,len(DataT[0])):
        ...
        for j in range(0,int(TrainingLen)):
            vct.append(Data[i][j])
        varVect.append(np.var(vct))
    noise = np.random.normal(0, 0.5*np.min(varVect), len(Data))
    for j in range(len(Data)):
...
Mu, Bigsigma = generate_mu_var('training.csv',singular_data)
trainingData, trainingLabel = processData('training.csv',singular_data, Mu, Bigsigma)
print("Training data all Processed:")
testingData, testingLabel = processData('testing.csv',singular_data, Mu, Bigsigma)
validationData, validationLabel = processData('validation.csv',singular_data, Mu, Bigsigma)
